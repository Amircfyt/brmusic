...
class Twenty_Twenty_One_Dark_Mode {
	...
	public function customizer_controls( $wp_customize ) {
		...
		if ( is_object( $colors_section ) ) {
			$colors_section->title = __( 'رنگ و حالت شب', 'twentytwentyone' );
		}
			...
			new Twenty_Twenty_One_Customize_Notice_Control(
				...
				array(
					...
					'active_callback' => function() {
						return 127 >= Twenty_Twenty_One_Custom_Colors::get_relative_luminance_from_hex( get_theme_mod( 'background_color', 'FFF' ) );
					},
				)
			)
		);
		...
		$description .= sprintf(
			...
			__( 'این قالب ساخته شده توسط امیر حسین چراغی<a href="%s"> دانلود از گیت هاب </a>', 'twentytwentyone' ),
			esc_url( __( 'https://github.com/amircfyt', 'twentytwentyone' ) )
		);
		$description .= '</p>';
		$description .= '<p>' . __( 'با فعال کردن حالت شب دکمه ای در وبسایت نمایش داده میشود که میتوان حالت شب را روی سایت فعال کرد', 'twentytwentyone' ) . '</p>';
		...
		$wp_customize->add_control(
			...
			array(
				'type'            => 'checkbox',
				'section'         => 'colors',
				'label'           => esc_html__( 'حالت شب', 'twentytwentyone' ),
				'priority'        => 110,
